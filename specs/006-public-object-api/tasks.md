...
- [X] T036 [US2] Replace the body of `CuemsScript.__json__` in `src/cuemsutils/cues/CuemsScript.py:292` with a one-line delegation to `to_wire()`, so the editor's existing `initial_template` call site receives the aligned payload before feature 009 migrates it — this is what closes F21 without editing a consumer repository. Pairs with T035, which leaves this one method for it (FR-012)
