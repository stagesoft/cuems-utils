...
- [X] T084 [P] Write `specs/006-public-object-api/migration-guide.md` from the D2 map: every removed entry point, its replacement, and the consumer sites in `cuems-engine`, `cuems-editor` and `cuems-nodeconf` — the deliverable feature 008 executes against. Consumer repositories are not edited here (FR-UX-001)
