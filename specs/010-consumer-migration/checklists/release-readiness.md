...
- [ ] CHK002 Is a requirement defined for **capturing the pre-migration payload** as a fixture? "Byte-identical to today's" is unverifiable after today unless a baseline is pinned before the migration starts [Gap, Spec §FR-010]
- [ ] CHK003 Are requirements defined for the raw-dict fixups still **detecting what they used to detect** — dangling targets, bad durations — or only for their new form and non-duplication? [Gap, Spec §FR-043]
...
| CHK002 | **"Byte-identical to today's" has no pinned baseline.** After the migration, "today" is gone. Nothing requires capturing the pre-migration payload as a fixture first, so the constraint most emphasised in this feature is currently the least verifiable |
| CHK003 | FR-043 requires the fixups to become sanctioned and to avoid duplicating library repairs — but **never requires they still catch what they caught** (dangling targets, bad durations). A conforming implementation could delete them all |
