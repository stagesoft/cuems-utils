...
`../cuems-wsclient` is a **stale checkout** still pointing at the pre-rename
...
**Every path in this file is relative to this repository's root** — `src/cuemsutils/` and `tests/`
here, and `../<repo>` for a sibling checkout beside this one, so `..` is the directory the
checkouts share. No absolute path appears on purpose: the layout is a convention, not a machine.
The same convention is used by the vendored bundles in `cuems-common` and `cuems-power-bridge`,
which is what makes a path quotable in either direction. If your checkouts are not siblings, these
paths are the only thing to adjust.
...
- [ ] T019 [P] [US1] Verify the `cuems-power-bridge` gate: its private network-map reader is gone (not re-spelled), and its first test fails against the old string comparison; record in `specs/010-consumer-migration/baseline.md` (FR-050, FR-051, SC-001, SC-007). **Same verification as T072/T073** — do it once, against `../cuems-power-bridge`, and never against the stale `../cuems-wsclient` checkout, which is 30 commits behind and frozen at 2026-06-01
...
- [ ] T023 [P] [US4] Verify each of the engine's four "keeps resolving but becomes wrong" sites (`../cuems-engine/src/cuemsengine/core/BaseEngine.py:33,410,440,443` — the four lines re-verified 2026-09-21 against a 636-line file; the original entry named the repository and the bare filename as two separate tokens, so the path was never written out) carries a test that **fails against the pre-migration value**; record the four in `specs/010-consumer-migration/baseline.md` (FR-004, SC-007)
...
- [ ] T025 [P] [US5] ⏸ **clause (b) CLEARED, clause (a) open** (see `baseline.md`): *no half-renamed combination is shippable* is **observed twice, independently** — `cuems-nodeconf`'s demonstration with real packages on both sides (N1/N2, and both single-half upgrades N5/N6), and, since 2026-09-21, `cuems-common`'s own after it closed its C1 gap (`f2fc0f5`) by rebuilding its stubs from `cuems-nodeconf`'s real `debian/control` at both versions instead of the floors-only approximation that produced the earlier ACCEPTED reading: B1/C1 refused in both directions, E1/F1 refused for either half alone, D1/G1 accepted for matched pairs. What remains is the **over-the-wire** check, which needs two hosts and now has a single named home — `cuems-nodeconf`'s `specs/002-public-network-map-path/checklists/hardware-verification.md`, which consolidates both its features' hardware debt including feature 001's T050 and T051. Verify the end-to-end discovery check ran — a node published by `../cuems-nodeconf`'s migrated publisher, discovered with its role by its migrated listener — and that no half-renamed combination is shippable; record in `specs/010-consumer-migration/baseline.md` (SC-012)
...
- [ ] T028 [P] [US6] Verify `../cuems-editor`'s `repair_durations.py` still reads the corrupt documents it exists to repair, and that exactly **one** document rewriter remains in the ecosystem; record the fixture set and result in `specs/010-consumer-migration/baseline.md` (FR-045, SC-010)
...
- [X] T030 [P] [US7] Verify 008's characterization tests in `../cuems-nodeconf` pass **unchanged** against the library's object — not edited to accommodate the new API; record the run in `specs/010-consumer-migration/baseline.md` (FR-065, SC-008)
...
- [ ] T032 [P] [US8] Verify the characterization tests for `../cuems-frontend`'s `projects.service.ts`, `project-edit/sequence/sequence.component.ts` and `settings.component.ts` were committed **before** the port and are green after it; record both commits in `specs/010-consumer-migration/baseline.md` (FR-084, D35, SC-013)
...
### Second upstream finding from `cuems-nodeconf` (received 2026-09-21, closed 2026-09-21)

`cuems-nodeconf`'s feature `002-public-network-map-path` reports one finding, again deliberately
unpatched from that side for the same reason: the yardstick's guarantee depends on this library's
files not being edited by a consumer. It is a **live defect and a merge blocker** — on the empty
map `cuems-common` ships, `NetworkMap.get_node` raised `TypeError` instead of the documented
`ValueError`, so `cuems-nodeconf` failed start-up every 10 s on every fresh install and only
`cuems-nodeconf` could write the map that would end the loop. Report:
`specs/010-consumer-migration/empty-node-list-report.md`; checklist: `empty-node-list-tasks.md`.

- [X] T085 [US7] Add `tests/contract/test_empty_node_list.py`, written failing-first against today's behaviour — five cases, all on inline XML constants so the test needs no sibling checkout (the empty document is byte-identical to what `../cuems-common` ships at `f2fc0f5`, 200 bytes): (a) `NetworkMap.get_node` on an **empty** `<node_list/>` raises `ValueError` whose message is still `Node with uuid <uuid> not found`; (b) the same for a document with **no** `node_list` element at all, the other `minOccurs="0"` shape; (c) `ConfigManager.load_network_map()` against an empty map raises `ValueError`, so the contract holds at the public boundary consumers actually use; (d) the empty document still **round-trips** — load, refill `node_list` with one node, `save`, re-load and find it; (e) a non-empty map still resolves a present uuid and still raises `ValueError` for an absent one. **Measured: 5 of 9 tests failed before T086, 9/9 pass after** — and the four that passed before are the evidence that `get_node` was the only unguarded path
- [X] T086 [US7] Guard the one unguarded iteration in `src/cuemsutils/xml/settings.py:158-160` (`NetworkMap.get_node`): `nodes_list = network_dict.get('node_list') or []`. **The trap avoided**: a `.get('node_list', [])` default does **not** work — an empty `<node_list/>` decodes to a key that is *present* with value `None`, so the default never fires; this is why the neighbouring code at `:187` and `:236` is safe by its `if not node_list:` check rather than by its default. The comment records that "no nodes at all" is the same answer as "no node with this uuid" — `ValueError`, which `ConfigManager.node_network_map` already documents and `../cuems-nodeconf`'s fresh-node path catches. Message unchanged (FR-1, FR-2)
- [X] T087 [P] [US7] Confirm no sibling path is unguarded — **re-measured after T086, not assumed**: `xml/settings.py:187` and `:236` raise their own `ValueError`; `config/network_map.py:181` `refresh` returns `False`/`True` correctly; `save` on an empty document writes. No `TypeError` anywhere, and the fresh-node sequence completes end to end. Recorded in `baseline.md`. **One adjacent finding recorded rather than fixed**: a document whose root is *entirely* empty (`<CuemsNetworkMap/>`) decodes to `None`, so `get_dict()` returns `{}` and `refresh`/`save` raise `AttributeError` — pre-existing, decode-layer, whole-document rather than `node_list`, and **not a shape anything ships**; `get_node` answers it correctly either way
- [X] T088 [P] [US7] Record the finding in `specs/010-consumer-migration/migration-guide.md` §4a-ii: the shipped map has had an empty `<node_list/>` since `../cuems-common`'s `f78c876`, so **the failing case was the default case on every node**, and `get_node`'s contract now holds for it. Includes why three suites missed it — XSD-only validation in `cuems-common`, no empty-map `ConfigManager` test here, non-empty fixtures in `cuems-nodeconf` — because that gap, not the one line, is the reusable lesson
...
**The findings document is `../cuems-power-bridge/specs/planning/cuems-power-bridge-node-role-findings.md`**
