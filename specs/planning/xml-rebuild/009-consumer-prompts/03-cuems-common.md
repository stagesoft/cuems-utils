# Feature 009 — `cuems-common`: conversion ordering, the packaging gate, half the Avahi cutover
...
outside this subset: read §2 of the prompts file above.
  ...
  D33 the Avahi TXT-record vocabulary (node_type=master|slave|firstrun) is renamed in BOTH
      this repository AND cuems-nodeconf, inside feature 009, as ONE coordinated cutover --
  ...
  D27 nothing in the ecosystem releases until every 009 flow lands. This repository is where
...
MEASURED STARTING STATE — verified against live files 2026-09-03, not transcribed:
  ...
  CLAUDE.md:88  says CONTROLLER_NETWORK_FLAG etc. are "migrated in feature 008" — the
      2026-08-25 renumbering makes that 009 (C9). Correct it.
...
- The postinst ordering is DECIDED AND WRITTEN DOWN, not inherited. The network-map
  ...
  007 deferred this here because the services doing the reading are the ones feature 009
...
- The release gate acquires the edges it is missing (C7). Today exactly ONE is mechanical:
  ...
  repositories existed; feature 009 is the release, so the excuse has expired. A gate that
...
- CLAUDE.md:88's "migrated in feature 008" is corrected to 009 (C9).
...
their MERGES are simultaneous (D33). Nothing here releases before every other 009 flow
...
until every 009 flow lands, and after this feature that sentence is a package
